...
async def remove_wallet(chat_id: int, address: str):
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        # Check if any other user is still monitoring this wallet
        # If not, clean up last_trades too
        if removed:
            ...
            # If nobody else monitoring, remove from last_trades
            if count == 0:
                await db.execute(
...
async def get_all_wallets():
    """Returns only currently monitored wallets freshly from DB"""
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute(
...
async def is_wallet_monitored(address: str) -> bool:
    """Check if a wallet is still being monitored by anyone"""
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute(
            "SELECT COUNT(*) FROM wallets WHERE address = ?",
            (address.lower(),)
